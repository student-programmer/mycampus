...
const ChatDetail = ({ user }: { user: User }) => {
    ...
    <div className={ style.wrapperDetail }>
      <div className={ style.goBackContainer }>
        ...
        <span className={ style.head_text }>
					<h2 className='h2'>{ receiverName } { receiverLastName }</h2>
          {/*{ currentChat.isOnline && <p className={ style.onlineText }>Online</p> }*/ }
          {/*{ !currentChat.isOnline &&*/ }
          {/*  <p className={ style.lastSeenText }>Offline</p> }*/ }
				</span>
        <Avatar style={ { border: '1px solid #FFFFFF29' } } src={ generateAvatar(receiverName, receiverLastName) }
                size={ 48 }/>
      </div>
          ...
          { messageList.map(item => (
              <div
                ...
                className={ `${ style.messageContainer } ${
                  item.senderId === user?.id ? style.myMessage : style.otherMessage
                }` }
              >
      ...
      <div className={ style.inputContainer }>
        ...
        />
        <button className={ style.sendBtn } data-active={ !!inputValue } onClick={ sendMessage }>
          <SendIcon/>
        </button>
      </div>
    </div>
